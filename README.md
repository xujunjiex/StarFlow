...
10. **底图与译文分层存储** — 只存原图 + 每页的译文文本与气泡坐标，**不存渲染好的译图**：磁盘占用从每页数百 KB 降到 KB 量级；改字号 / 颜色 / 竖排方向 / 译文替换规则后，已翻译页面**直接重新渲染，不用重新翻译**
11. **译文替换表** — 自定义多条「查找 → 替换」规则（例如把 `...` 换成 `.`），在设置里随时增删改；规则在渲染译文时套用，改完即生效
12. **悬浮球可调大小与不透明度** — 50%–200% 大小、10%–100% 不透明度，游戏与漫画共用一份设置，在屏的球实时跟随
...
悬浮窗覆盖翻译，支持像素驱动自动翻译和手动框选两种模式。悬浮球单击/双击/长按可自由分配动作（翻译/菜单/自动翻译）。悬浮球的大小（50%–200%）与不透明度（10%–100%）可在个性化设置里调，游戏与漫画共用同一份设置，拖动即时生效（球变大贴边时会自动夹回屏幕内，避免滑出可视区域摸不到）。
...
- **导入即显示进度** — 选完立刻出现一张占位卡片（图片位显示进度条与百分比），可随时点 ✕ 取消；导入在后台进行，旋转 / 切页不会中断。
- **导入整个文件夹 = 一部漫画** — 文件夹（含各级子文件夹）里的图片按「先目录名、再夹内页名」的自然顺序拼成一整本；夹里的压缩包不会被识别，要一次导入多部请用「导入文件（zip/cbz）」多选。
- **删除前会提醒** — 选中的书里有译文时会提示「删除后无法恢复」，并给一个「先去导出」入口（可先到阅读器里打包下载译文 / 双语）。
...
**阅读器内嵌翻译**：手动 / 自动 / 增量三种模式共用一条串行队列（翻页不打断、只翻当前在看的页上屏），译文实时叠加在页面上，支持译文 / 原文 / 纯原图三态切换（连续滑动模式为原图 / 译文两态）；失败页显示感叹号，点开看失败原因。译文字号（自动 / 8~48 档）在个性化设置、悬浮窗菜单与翻译面板三处共用同一份设置。

- **底图与译文分层** — 翻译结果只以「文本 + 气泡坐标」入库，**渲染好的译图不落盘**：改字号、颜色、竖排方向、字距行距、重叠合并或译文替换规则时，已翻译页面**直接按当前设置重新渲染**，既不用重新识别/翻译，也不用重新截图；三态切换与三种打包下载都由这一份数据按需生成。代价只是打开页面时即时渲染一次（毫秒级，有内存缓存）。
- **译文替换表**（个性化设置 → 漫画翻译结果设置 → 译文替换表）— 可自由增删多条「查找 → 替换」规则，例如把识别到的 `...` 统一换成 `.`；规则在渲染译文时套用，所以**改完回阅读器即生效，不必重翻**（已经翻译过的页面同样生效）。
- **省略号不再被拆行** — 模型偶尔会把一条译文写成好几行（`.\n.\n.`），译文渲染前会把这些「点与点之间的换行」接回同一行，并且排版时点串整体不拆分，不会再出现「每行一个点」占满气泡、把字号挤小的情况。
