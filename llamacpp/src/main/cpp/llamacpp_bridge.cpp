// llamacpp_bridge.cpp — llama.cpp 通用本地推理 JNI 桥（llama.cpp 源码自编，见 patches/README.md）
// 引擎: submodule pin = PR #22836 head（43 = STQ1_0，即腾讯 Hy-MT2 1.25-bit 量化）
#include <jni.h>
...
Java_translationapi_llamacpp_LlamaCppNative_nativeSetLogFile(
...
extern "C" JNIEXPORT void JNICALL
Java_translationapi_llamacpp_LlamaCppNative_nativeTriggerNativeCrash(
    JNIEnv* env, jclass, jstring jPath) {
    try {
        const char* path_c = env->GetStringUTFChars(jPath, nullptr);
        if (path_c == nullptr) return;
...
Java_translationapi_llamacpp_LlamaCppNative_nativeInit(
    ...
    try {
        ...
        mp.n_gpu_layers = 0;
        // ⚠️ LLAMA_LOAD_MODE_NONE = 不用 mmap、不用 mlock（等价于旧的 use_mmap=false + 跳过 mlock）。
        //    上游已把 use_mmap/use_mlock 两个 bool 换成 llama_load_mode 枚举，不改这里根本编不过。
        //    理由（2026-09 上游源码审计）：
        //      - mmap 的权重页会被系统在内存压力下回收，解码时反复从存储读 → 旧实测 15-17s/次；
        //      - mlock 在新版是软失败（上游只 WARN、不会让加载失败），且真机实测 RLIMIT_MEMLOCK
        //        只有 64KB，锁 440MB 必然失败 → 旧的手写 mlock_capable() 是假阳性，已删。
...
static jstring translate_impl(JNIEnv* env, jlong jHandle, jstring jPrompt, jstring jPrefix,
    ...
    try {
        ...
        // 上游新签名多了第一个参数 n_vocab（旧版没有）—— 不改编不过
        llama_sampler_chain_add(smpl, llama_sampler_init_penalties(
            llama_vocab_n_tokens(h->vocab), 64, repetitionPenalty, 0.0f, 0.0f));
...
Java_translationapi_llamacpp_LlamaCppNative_nativeTranslate(
...
extern "C" JNIEXPORT jstring JNICALL
Java_translationapi_llamacpp_LlamaCppNative_nativeTranslateStreaming(
    JNIEnv* env, jclass, jlong jHandle, jstring jPrompt, jstring jPrefix,
    jfloat temperature, jfloat topP, jint topK,
...
extern "C" JNIEXPORT void JNICALL
Java_translationapi_llamacpp_LlamaCppNative_nativeRelease(JNIEnv*, jclass, jlong jHandle) {
    try {
        std::lock_guard<std::mutex> lock(g_mutex);
        auto* h = reinterpret_cast<LlamaCppHandle*>(jHandle);
        if (h == nullptr) return;
...
extern "C" JNIEXPORT void JNICALL
Java_translationapi_llamacpp_LlamaCppNative_nativeAbort(JNIEnv*, jclass, jlong jHandle) {
    auto* h = reinterpret_cast<LlamaCppHandle*>(jHandle);
    if (h != nullptr) {
        h->abort.store(true);
...
extern "C" JNIEXPORT jstring JNICALL
Java_translationapi_llamacpp_LlamaCppNative_nativeTranslateChat(
